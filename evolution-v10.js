(function(){
  ...
  renderPlayer=function(owner,el){
    const s=livingSummary(owner),systemTag=singlePlayer&&owner==='amber'?'<span class="system-player-tag">Sistema</span>':'';
    el.innerHTML=`
  ...
  renderActions=function(){
    ...
    const hint=$('#hint');
    if(singlePlayer&&state?.current==='amber'&&!state.gameOver)hint.textContent='Turno das Pretas · Sistema.';
  ...
  function moveScore(org,t){
    const defender=organismAt(t.r,t.c),dest=cell(t.r,t.c),p=profileOf(org);normalizeProfile(p);
    const lethal=!has(org,'Voo')&&Array.isArray(t.path)&&t.path.some(([r,c])=>cell(r,c).terrain==='biohazard');
  ...
  function bestSystemMove(){
    ...
    if(state.moveChain){
      ...
      if(org)for(const target of movementTargets(org))moves.push({org,target,score:moveScore(org,target)});
    }else{
      for(const org of playerOrganisms('amber'))for(const target of movementTargets(org))moves.push({org,target,score:moveScore(org,target)});
    }
    if(!moves.length)return null;
    moves.sort((a,b)=>b.score-a.score);
    const topScore=moves[0].score,top=moves.filter(m=>m.score>=topScore-3);
    return choice(top);
  }
  ...
  if(rules[5])rules[5].innerHTML='<strong>Controles.</strong> Use o botão de modo no topo para alternar entre 2 jogadores e 1 jogador. No modo de 1 jogador, você controla as Brancas e o sistema controla as Pretas. Clique numa peça sua e depois numa casa com borda azul; “Passar a vez” encerra seu turno sem movimento.';
