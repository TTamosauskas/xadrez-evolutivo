...
function earthTimelineLines() {
  return GEOLOGICAL_STAGES.map((stage) => {
    if (stage.id === "hadean")
      return `${stage.group} · ${stage.period}: prólogo tutorial — deslocar → dividir → capturar; termina por extinção`;
    if (stage.id === "archean")
      return `${stage.group} · ${stage.period}: 1º Ciclo — Respiração anaeróbia basal / Fotossíntese / Predação; 2º Ciclo — Reparo Celular; 3º Ciclo — Dormência`;
...
export function howToPlayLines() {
  ...
  return [
    ...
    section("Hadeano"),
    "O Hadeano é o prólogo da campanha Vida na Terra. Um único Rei cinza representa o ancestral comum; ao ser ativado, divide-se em um Rei branco e um Rei preto, ainda sem mutações adquiridas. O núcleo 4×4 é fértil, mas as duas camadas externas ☠️ continuam fazendo parte do tabuleiro: podem ser ocupadas e são letais, ensinando a hostilidade ambiental pela consequência.",
    "Durante o Hadeano não surgem mutações aleatórias. Deslocar, dividir e capturar são objetivos pedagógicos, mas não encerram a fase. Como nos demais períodos, o Hadeano termina apenas quando um dos lados é extinto; então ocorre a transição formal para Arqueano · 1º Ciclo.",
    ...
    "Vida na Terra: campanha histórica. O Hadeano funciona como prólogo tutorial hipotético e termina por extinção de um dos lados. O Arqueano começa sem mutações adquiridas: todas as linhagens mantêm apenas Respiração anaeróbia basal, com zero mutações, e podem seguir três possibilidades metabólicas — permanecer anaeróbias, adquirir 🟢 Fotossíntese ou adquirir 👾 Predação.",
    "Abertura geológica: no Hadeano, o núcleo 4×4 é fértil e as duas camadas externas ☠️ permanecem jogáveis, mas são letais. No Arqueano · 1º Ciclo, o espaço habitável expande para um núcleo fértil 6×6 cercado por uma camada 🟥 hostil; Fotossíntese e Predação ficam abertas simultaneamente, enquanto Respiração anaeróbia permanece como estratégia basal. No 2º Ciclo, o tabuleiro aquático torna-se integralmente fértil e Reparo Celular passa a ser a inovação didática principal. No 3º Ciclo, Dormência é liberada. Só depois dessas ondas o jogo avança ao Proterozoico.",
