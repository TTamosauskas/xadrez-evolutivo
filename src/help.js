...
export function howToPlayLines() {
  ...
  return [
    ...
    `Surtos usam três agentes em uma progressão didática: Vírus a partir do Proterozoico, Bactérias a partir do Ediacarano e Fungos a partir do Cambriano. Vírus comuns se espalham entre criaturas; Bactérias deixam rastros infecciosos nas casas abandonadas por hospedeiros; Fungos contaminam casas e criam dois novos focos distantes por rodada de surto.`,
    `No ciclo seguinte à primeira descoberta de ${traitLabel("Reprodução Sexuada")}, surtos ecológicos também podem assumir uma rota viral sexual: a transmissão ocorre apenas durante acasalamento e não por mera proximidade. ${traitLabel("Resistência")} impede completamente essa infecção; ${traitLabel("Imunodeficiência")} anula a proteção.`,
    `Cada exposição efetiva pode causar uma alteração somática negativa, limitada a uma por surto e não herdável. ${traitLabel("Reparo Celular")} reduz essa chance; ${traitLabel("Resistência")} impede infecção por patógenos ecológicos e reduz a mortalidade de surtos ligados à pressão populacional.`,
