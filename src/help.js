...
export function howToPlayLines() {
  ...
  return [
    ...
    "O Menu permite salvar a partida, importar um arquivo, consultar História evolutiva, Descobertas e Log da partida. Como o jogo está em desenvolvimento, saves pertencem somente ao schema atual e versões antigas são consideradas incompatíveis. História evolutiva mostra o período, o Ciclo e as próximas inovações; Descobertas reúne eras, eventos e mutações já observadas; o Log registra mudanças relevantes em ordem cronológica reversa.",
    "No tabuleiro, apenas características fenotípicas diferenciais formam a moldura de até 12 ícones ao redor da peça. Uma característica hereditária só passa ao Legado Genético quando está expressa em todas as peças vivas do tabuleiro; se surgir qualquer peça sem ela, retorna automaticamente à exibição individual. Predação, Fotossíntese e características próprias de ramos seguem essa mesma regra global. No painel selecionado, as descrições são resumos operacionais; mutações que produzem uma ação legal naquele turno aparecem primeiro e em negrito. Efeitos que progridem ou podem ser acionados ao permanecer estacionário também contam como acionáveis enquanto suas condições estiverem válidas. O catálogo abaixo preserva as descrições completas. Vantagens e desvantagens diferenciais ficam separadas; Genes Recessivos permanecem ocultos no fenótipo, e mutações somáticas continuam sempre visíveis como alterações individuais. Círculos vermelhos indicam Ataques disponíveis, incluindo tentativas de captura, Ovífagia, Canibalismo, predação botânica e Parasitismo dirigido. Círculos verdes indicam Vivificar: reprodução, parceiros sexuais, Brotamento, Metamorfose e fertilização da própria casa por Parasitismo. Quando mais de uma forma de Vivificar está disponível na mesma casa, o jogo pede uma escolha curta. Como ataques podem falhar por defesas, o círculo vermelho indica a tentativa hostil, não uma captura garantida. A legenda sob o tabuleiro é contextual: mostra apenas terrenos, barreiras, decomposição e sinais de ação que estejam visíveis naquele momento.",
